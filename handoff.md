...
**Opgeleverd op deze branch:** de Gemini-workflow eist één exacte verdict-trailer, bindt de review aan de gecontroleerde PR-head en plaatst via `pulls.createReview` een formele `APPROVE` of fail-closed `REQUEST_CHANGES`. De auto-advance-evaluator accepteert alleen een expliciete `APPROVED`-review van `github-actions[bot]` naast de drie groene checks; ontbrekende, ongeldige of vreemde reviewmetadata blijft no-op. De attended-runbookstap voor GitHubs bot-approvalinstelling is toegevoegd. Bewijs: 83 Node-tests, 91 Python-tests, YAML-, handoff-, architectuur-, release- en PII-gates groen; de bekende Windows Job Object-timingrace was bij gerichte en volledige herhaling groen. `PIPELINE_AUTOMERGE` blijft uit; deploy/productie worden niet geraakt.
