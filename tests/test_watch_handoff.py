...
class WatchHandoffAutorunTests(unittest.TestCase):
    ...
    def test_nonzero_runner_block_finalizes_via_steward_then_notifies(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("runner eindigde met exitcode 9", blocked["note"])
            ...
            self.assertEqual(notifications, [repo])
            self.assertIn(
                "runner eindigde met exitcode 9",
                (repo / "autorun.log").read_text(encoding="utf-8"),
            )
    ...
    def test_pipeline_setup_guard_passes_through_concrete_error(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = self.make_repo(directory)
            metadata = repo / "config" / "pipeline-sprint.json"
            metadata.parent.mkdir()
            ...
            completed = subprocess.CompletedProcess(
                ["pipeline_guard"], 2, stdout="", stderr="PIPELINE_GUARD_BLOCKED: open PR #24 moet eerst sluiten\n"
            )
            with mock.patch.object(watch_handoff.subprocess, "run", return_value=completed):
                with self.assertRaisesRegex(watch_handoff.AutorunError, "open PR #24"):
                    watch_handoff.run_pipeline_setup_guard(repo)
